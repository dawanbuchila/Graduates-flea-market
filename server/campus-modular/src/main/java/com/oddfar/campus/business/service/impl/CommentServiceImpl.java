...
//        comment.setPageNum(firstIndex);
        ...
        if (content != null) {
            ...
            // 递归将所有的节点设置为匿名节点
            if (content.getIsAnonymous() == 1) {
                this.setNicknameAndAvatar(oneLevel);
            }
...
//            return new PageResult<CommentVo>(oneLevel, selectCommentCount(content.getContentId()));
    ...
    // =======================新添加的代码=========================
    private void setNicknameAndAvatar(List<CommentVo> commentVoList) {
        for (CommentVo commentVo : commentVoList) {
            // 设置nickname和avatar
            commentVo.setUserNickName("匿名用户");
            commentVo.setToUserNickName("匿名用户");
            commentVo.setAvatar("https://gcore.jsdelivr.net/gh/oddfar/static/campus/image/anonymous.jpeg");

            // 如果该评论有子评论，则递归调用该方法对其子评论也进行设置
            if (commentVo.getChildren() != null && !commentVo.getChildren().isEmpty()) {
                setNicknameAndAvatar(commentVo.getChildren());
            }
        }
    }
    ...
    public PageResult<CommentVo> selectOneLevelChild(CommentEntity comment) {
        ...
        // 00000000000000000000000000000新添加的代码000000000000000000000000000
        setNicknameAndAvatar(oneLevelChild);
        // 00000000000000000000000000000新添加的代码000000000000000000000000000
